...
let fileStats : iStats, runntime : number = 0;
...
TESTE.action(async(options: any)=>{
       ...
       const Name = 'rodcordeiro' , fileName = Buffer.from(Name).toString('base64'), tempDir = tmpdir(), file = resolve(`${tempDir}/${fileName}.txt`)
       ...
       const watcher = await watch(file,{
        ...
        watcher.on('all', ( event: any, path: any, stats: iStats) => {
            console.log({stats,runntime})
            if (event == 'add' && runntime === 0 || event == 'change' && runntime === 0){
                fileStats = stats
                runntime++;
