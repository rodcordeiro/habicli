...
import { PRIORITIES, ATTRIBUTE, iTag, iChecklistItem } from '../utils';
...
async function complete(id: string | undefined, headers: { [k: string]: any }) {
...
async function getTags(headers: { [k: string]: any }) {
  return new Promise(async (resolve, reject) => {
    await api
      .get(`/tags`, {
        headers,
...
async function completeChecklistItem(
  ...
  return new Promise(async (resolve, reject) => {
    await api
      .post(
        `/tasks/${task_id}/checklist/${item_id}/score`,
        {},
        {
          headers,
        },
      ...
      .then(async (response: any) => {
        ...
        response.data.data.map((task: any) => {
          tasks.push({
            ...
            tags: task.tags.map(
              (tag: string) => uTags.filter((t: iTag) => t.id == tag)[0],
            ),
            type: task.type,
...
const Todo = new Command('todo')
  ...
  .helpOption(
    '-h,--help',
    'Provides todo functionallity. Allows to list, edit, update, score, create and delete todo tasks',
  )
  .action(async (options) => {
    ...
    if (!headers) {
      ...
      const task: iTask = tasks.filter(
        (task: iTask) => task.text == choose.task,
      )[0];
      ...
      const action_choices = ['Complete', 'Update', 'Delete', 'Close'];
      ...
      switch (action) {
        case 'Checklist':
          ...
          const item_action = await inquirer
            .prompt([
              {
                type: 'list',
                name: 'action',
                message: 'What are we going to do?',
                choices: ['Complete', 'Update', 'Delete', 'Exit'],
              },
            ])
            .then((act) => {
              return act.action;
          ...
          if (item_action == 'Update') {
            ...
            await api
              .put(
                `/tasks/${task.id}/checklist/${item.id}`,
                {
                  text,
                },
                { headers },
          ...
          if (item_action == 'Delete') {
            await api
              .delete(`/tasks/${task.id}/checklist/${item.id}`, { headers })
              .then((response) => {
                console.log(chalk.cyanBright('Updated'));
              })
              .catch((err) => {
        ...
        case 'Update':
          const { choice } = await inquirer.prompt([
            {
              ...
              choices: ['Title', 'Notes', 'Tags', 'Add checklist item'],
            },
          ]);
          if (choice == 'Title') {
          ...
          if (choice == 'Tags') {
            console.log(
              `Existing tags are:${task.tags.map(
                (tag: iTag) => ` ${tag.name}`,
              )}`,
            );
            let { choice } = await inquirer.prompt([
              {
                ...
                choices: ['Add tags', 'Remove tags', 'Exit'],
              },
            ]);
            if (choice == 'Add tags') {
              ...
              await inquirer
                ...
                .then(async (response) => {
                  if (response.tag != 'Empty') {
                    let tag = tags.filter(
                      (t: iTag) => t.name == response.tag,
                    )[0];
                    await api
                      .post(`/tasks/${task.id}/tags/${tag.id}`, {}, { headers })
              ...
              await inquirer
                ...
                .then(async (response) => {
                  if (response.tag != 'Empty') {
                    ...
                    await api
                      .delete(`/tasks/${task.id}/tags/${tag.id}`, { headers })
                      .catch((err) => console.log(chalk.redBright(err)));
                  } else {
                    exit();
                  }
                ...
                .prompt([
                  {
                    ...
                    name: 'text',
                    message:
                      'Please, type the item text: [leave it empty for exit]',
                    default: null,
                  },
                ])
                .then((response) => response.text);
...
Todo.command('create')
  ...
  .action(async (options) => {
    ...
    if (!headers) {
        ...
        : await inquirer.prompt([
            {
              ...
              name: 'notes',
              message:
                'Do you wish to add some notes? [Press enter to leave blank]',
              default: false,
            },
          ]);
      ...
      await api
        .post('/tasks/user', task, { headers })
        .then((response) => {
          if (!options.verbose) {
            spinner.succeed(chalk.cyanBright('Task created'));
          } else {
            spinner.succeed(
              chalk.cyanBright(JSON.stringify(response.data.data)),
            );
          }
        })
        .catch((err) => {
